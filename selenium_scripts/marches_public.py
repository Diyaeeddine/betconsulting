...
# --- CRÉER UN FICHIER DE LOGS POUR DEBUGGING ---
log_file_path = os.path.join(result_dir, "python_execution.log")

def log_message(message):
    """Fonction pour logger dans le fichier ET stdout"""
...
log_message(f"[INFO] Storage directory: {result_dir}")
log_message(f"[INFO] Log file: {log_file_path}")
...
# --- CRÉER UN FICHIER DE STATUT POUR LARAVEL ---
status_file = os.path.join(result_dir, "execution_status.json")
...
def update_status(status, message=""):
    ...
    try:
        ...
        log_message(f"[STATUS] {status}: {message}")
    except Exception as e:
        log_message(f"[ERROR] Cannot update status: {e}")
...
if os.path.exists(existing_references_file):
    try:
        ...
        log_message(f"[INFO] {len(existing_references)} existing references loaded to avoid duplicates")
    except Exception as e:
        log_message(f"[WARN] Cannot load existing references: {e}")
        existing_references = set()
else:
    log_message("[INFO] No existing references file found - all markets will be processed")
...
try:
    ...
    log_message("[INFO] Chrome driver initialized successfully")
except Exception as e:
    log_message(f"[ERROR] Cannot initialize Chrome driver: {e}")
    update_status("error", f"Chrome initialization failed: {e}")
    sys.exit(1)
...
def is_duplicate_reference(reference):
    ...
    if is_dup:
        log_message(f"[DUPLICATE] Reference {reference_clean} already in database - SKIP download/extraction")
...
def extract_zip_file(zip_path, reference):
    ...
    try:
        if not os.path.exists(zip_path):
            log_message(f"[ERROR] ZIP file not found: {zip_path}")
            return False, []
            
        if os.path.getsize(zip_path) == 0:
            log_message(f"[ERROR] Empty ZIP file: {zip_path}")
            return False, []
        
        if os.path.exists(extract_dir):
            try:
                shutil.rmtree(extract_dir)
                log_message(f"[INFO] Old extraction folder removed: {extract_dir}")
            except Exception as e:
                log_message(f"[WARN] Cannot remove old folder: {e}")
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as test_zip:
                ...
                if bad_file:
                    log_message(f"[ERROR] Corrupted ZIP, faulty file: {bad_file}")
                    return False, []
        except zipfile.BadZipFile:
            log_message(f"[ERROR] Invalid or corrupted ZIP file: {zip_path}")
            return False, []
        except Exception as e:
            log_message(f"[ERROR] Error testing ZIP: {e}")
            return False, []
        ...
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            ...
            if not extracted_files:
                log_message(f"[ERROR] No files extracted from ZIP: {reference}")
                return False, []
            
            log_message(f"[INFO] ZIP extracted successfully: {reference}")
            log_message(f"[INFO] → Folder: {extract_dir}")
            log_message(f"[INFO] → {len(extracted_files)} files extracted")
            
            return True, extracted_files
            
    except Exception as e:
        log_message(f"[ERROR] Error extracting {reference}: {str(e)}")
        if os.path.exists(extract_dir):
            try:
                shutil.rmtree(extract_dir)
            except:
...
def extract_table_data():
    ...
    try:
        ...
        log_message(f"[INFO] Table loaded with {len(rows)} rows")
        ...
        for i, row in enumerate(rows):
            try:
                ...
                if len(cells) >= 5:
                    ...
                    log_message(f"[INFO] Row {i+1} extracted: {row_data['reference']}")

            except Exception as e:
                log_message(f"[ERROR] Error extracting row {i+1}: {str(e)}")
                continue

    except Exception as e:
        log_message(f"[ERROR] Error extracting table: {str(e)}")

    return data

def save_to_csv(data, filename="marches_publics_data.csv"):
    if not data:
        log_message("[WARN] No data to save to CSV")
        return
    ...
    log_message(f"[INFO] CSV saved to {filepath} ({len(data)} rows)")
    return filepath

def save_to_json(data, filename="marches_publics_data.json"):
    if not data:
        log_message("[WARN] No data to save to JSON")
        return
    filepath = os.path.join(result_dir, filename)
    with open(filepath, 'w', encoding='utf-8') as jsonfile:
        json.dump(data, jsonfile, ensure_ascii=False, indent=2)
    log_message(f"[INFO] JSON saved to {filepath} ({len(data)} rows)")
    return filepath
...
def cancel_active_downloads():
    try:
        ...
        """)
        log_message("[INFO] Active downloads cancelled.")

    except Exception as e:
        log_message(f"[WARN] Cannot cancel downloads: {e}")
    ...
    for item in data:
        if item.get('reference') == reference:
            if zip_path and os.path.exists(zip_path):
                ...
                log_message(f"[INFO] Data updated - chemin_zip for {reference}")
            
            if extracted_files:
                item['EXTRACTED_FILES'] = extracted_files
                log_message(f"[INFO] Data updated - {len(extracted_files)} extracted files for {reference}")
...
def download_dce(row, data):
    ...
    if is_duplicate_reference(reference):
        ...
        log_message(f"[SKIP] Reference {reference} ignored (duplicate detected)")
        return

    try:
        if not row.get('lien_consultation'):
            log_message(f"[ERROR] No consultation link for {reference}")
            fail_count += 1
            return
        ...
        try:
            ...
            log_message(f"[INFO] Download button clicked for {reference}")
        except Exception as e:
            log_message(f"[WARN] Cannot click Download for {reference}: {e}")
            fail_count += 1
            driver.close()
            driver.switch_to.window(driver.window_handles[0])
            return
        ...
        log_message(f"[INFO] Waiting for download for {reference}...")
            ...
            if cr_files:
                ...
                if current_size == last_size:
                    if (time.time() - last_check) > 30:
                        stall_count += 1
                        log_message(f"[WARN] Download stalled ({stall_count}/3) for {reference}")
                        if stall_count >= 3:
                            log_message(f"[ERROR] Download permanently stalled for {reference}")
                            cancel_active_downloads()
                            if os.path.exists(cr_file):
                                try:
                                    os.remove(cr_file)
                                    log_message("[INFO] .crdownload file removed")
                                except:
                                    pass
                            fail_count += 1
                            return
                    ...
                    stall_count = 0
                    if current_size > 0:
                        log_message(f"[INFO] Download in progress... ({current_size} bytes)")
        ...
        if downloaded_file:
            log_message(f"[INFO] File downloaded: {os.path.basename(downloaded_file)}")
            ...
            try:
                ...
                log_message(f"[INFO] ZIP renamed: {new_name}")
            except Exception as e:
                log_message(f"[WARN] Error renaming: {e}")
                new_path = downloaded_file

            time.sleep(2)

            log_message(f"[INFO] Starting extraction for {reference}")
            success_extract, extracted_files = extract_zip_file(new_path, reference)
            
            if success_extract:
                extract_success_count += 1
                log_message(f"[INFO] Extraction successful for {reference}")
                update_data_with_files(data, reference, new_path, extracted_files)
            else:
                extract_fail_count += 1
                log_message(f"[ERROR] Extraction failed for {reference}")
                update_data_with_files(data, reference, new_path)

            success_count += 1
            log_message(f"[INFO] Download successful for {reference}")
            
        else:
            log_message(f"[ERROR] Timeout ({timeout}s) for {reference}")
            cancel_active_downloads()
            fail_count += 1

    except Exception as e:
        log_message(f"[ERROR] General error for {reference}: {str(e)}")
        cancel_active_downloads()
        fail_count += 1
...
def process_existing_zips(data):
    ...
    zip_files = glob.glob(os.path.join(result_dir, "*.zip"))
    if not zip_files:
        log_message("[INFO] No ZIP files found")
        return
    
    log_message(f"[INFO] {len(zip_files)} ZIP files found")
    ...
    for zip_path in zip_files:
        ...
        if is_duplicate_reference(reference):
            log_message(f"[SKIP] ZIP ignored (duplicate): {reference}")
            continue
        
        extract_dir = os.path.join(result_dir, f"{reference}_extrait")
        if os.path.exists(extract_dir) and os.listdir(extract_dir):
            log_message(f"[INFO] {reference} already extracted, updating data...")
            extracted_files = []
            for root, dirs, files in os.walk(extract_dir):
                for file in files:
                    file_path = os.path.join(root, file)
        ...
        log_message(f"[INFO] Extracting {reference}...")
...
if __name__ == "__main__":
    try:
        update_status("running", "Script started")
        log_message("[INFO] === Starting public markets extraction ===")
        log_message(f"[INFO] Existing references loaded: {len(existing_references)}")
        start_time = time.time()

        # --- AUTHENTICATION ---
        update_status("running", "Authenticating...")
        ...
        bouton_login.click()
        log_message("[INFO] Clicked on 'S'identifier'")
        ...
        log_message("[INFO] Username entered")
        ...
        log_message("[INFO] Password entered")
        ...
        update_status("running", "Navigating to search page...")
        log_message("[INFO] Navigating to advanced search...")
        ...
        log_message("[INFO] Advanced search page loaded")
        
        time.sleep(2)

        # Click the search button
        log_message("[INFO] Clicking search button...")
        ...
        time.sleep(1)
        bouton.click()
        log_message("[INFO] Search button clicked")

        # --- WAIT FOR RESULTS TABLE ---
        update_status("running", "Loading results...")
        log_message("[INFO] Waiting for results table...")
        WebDriverWait(driver, 30).until(
            EC.presence_of_element_located((By.CSS_SELECTOR, "table.table-results tbody tr"))
        )
        log_message("[INFO] Results table loaded successfully")

        # Set results per page
        log_message("[INFO] Setting results per page to 10...")
        ...
        time.sleep(3)
        log_message("[INFO] Results per page set to 10")

        # Extract table data
        update_status("running", "Extracting table data...")
        log_message("[INFO] Extracting table data...")
        data = extract_table_data()

        if data:
            log_message(f"[INFO] {len(data)} markets extracted from table")
            ...
            log_message(f"[INFO] {len(filtered_data)} new markets after duplicate filtering")
            log_message(f"[INFO] {len(data) - len(filtered_data)} duplicates detected and ignored")
            
            # Process existing ZIPs
            update_status("running", "Processing existing ZIP files...")
            process_existing_zips(data)
            
            # Download and extract new markets
            log_message(f"\n[INFO] === Starting download of {len(filtered_data)} new markets ===")
            update_status("running", f"Downloading {len(filtered_data)} new markets...")
            
            for i, row in enumerate(filtered_data, 1):
                log_message(f"\n[INFO] === New market {i}/{len(filtered_data)} ===")
                update_status("running", f"Processing market {i}/{len(filtered_data)}: {row.get('reference', 'N/A')}")
                download_dce(row, data)
                time.sleep(1)

            duration = time.time() - start_time
            log_message(f"\n[INFO] === DOWNLOAD/EXTRACTION SUMMARY WITH DUPLICATE FILTERING ===")
            log_message(f"[INFO] Total duration: {duration:.2f} seconds")
            log_message(f"[INFO] Markets in table: {len(data)}")
            log_message(f"[INFO] Duplicates ignored: {skipped_duplicates_count + (len(data) - len(filtered_data))}")
            log_message(f"[INFO] New markets processed: {len(filtered_data)}")
            log_message(f"[INFO] Successful downloads: {success_count}")
            log_message(f"[INFO] Failed downloads: {fail_count}")
            log_message(f"[INFO] Successful extractions: {extract_success_count}")
            log_message(f"[INFO] Failed extractions: {extract_fail_count}")
            
            # Generate final CSV and JSON files
            log_message(f"\n[INFO] === GENERATING FINAL CSV/JSON FILES ===")
            update_status("running", "Generating CSV and JSON files...")
            save_to_csv(data)
            save_to_json(data)
            
            log_message(f"\n[INFO] === FINAL FILES GENERATED WITH DUPLICATE OPTIMIZATION ===")
            log_message(f"[INFO] Final CSV generated with all data")
            log_message(f"[INFO] Final JSON generated with all data") 
            log_message(f"[INFO] All ZIP paths and extracted files are included")
            log_message(f"[INFO] Time and bandwidth saved by avoiding duplicates")
            
            update_status("completed", f"Successfully processed {len(filtered_data)} new markets")
            
        else:
            log_message("[WARN] No data extracted")
            update_status("completed", "No data extracted")

    except TimeoutException as e:
        error_msg = f"Timeout error: {str(e)}"
        log_message(f"[ERROR] {error_msg}")
        log_message("[ERROR] The page took too long to load. Possible causes:")
        log_message("[ERROR] - Website is slow or down")
        log_message("[ERROR] - Network connection issues")
        log_message("[ERROR] - Incorrect page elements or selectors")
        update_status("error", error_msg)
        
    except NoSuchElementException as e:
        error_msg = f"Element not found: {str(e)}"
        log_message(f"[ERROR] {error_msg}")
        log_message("[ERROR] The page structure may have changed")
        log_message("[ERROR] Check if selectors are still valid")
        update_status("error", error_msg)
        ...
        log_message(f"[ERROR] {error_msg}")
        import traceback
        log_message(f"[ERROR] Stack trace:")
        log_message(traceback.format_exc())
        update_status("error", error_msg)

    finally:
        log_message("\n[INFO] === Script finished ===")
        log_message("[INFO] Check the logs above for any errors")
        log_message(f"[INFO] Files saved to: {result_dir}")
        log_message(f"[INFO] Log file: {log_file_path}")
