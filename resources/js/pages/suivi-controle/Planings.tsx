...
import { Plus, Save, MapPin, Users, Calendar, Target, Eye, CreditCard as Edit2, Trash2, UserPlus, UserMinus, CheckCircle, Clock, AlertCircle, XCircle, Filter, Zap, Phone, FileText, X, Bell, ChevronDown, ChevronRight, ChevronLeft, ArrowLeft, ZoomIn, ZoomOut, CalendarDays, Download, ClipboardList, Map, BarChart3 } from 'lucide-react';
...
export default function PlanningManagement() {
  ...
  const deactivateNotification = async (notificationId: number) => {
    ...
    try {
      const response = await fetch(`/suivi-controle/notif/${notificationId}`, {
        method: 'PUT',
        headers: {
  ...
  // Form handlers
  const handleCreatePlans = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setIsSubmitting(true);
    setError(null);

    console.log('Creating plans with payload:', planFormData);

    // TODO: Implement backend submission logic
    addMessage('info', 'Fonctionnalité de création en cours de développement');
                    ...
                    {sousPlans.length > 0 && (
                      <div className="ml-8 mt-2 space-y-1">
                        {sousPlans.map((sousPlan, spIndex) => {
                          // Calculate sous-plan dates based on duration
                          // For display purposes, we'll show them sequentially
                          return (
    ...
    <AppLayout breadcrumbs={breadcrumbs}>
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          {/* Create/Edit Plan Popup - NEW STRUCTURE */}
